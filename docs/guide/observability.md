...
`MetricsPlugin` turns the agent loop into OpenTelemetry spans: one span per tool call
(tagged with the tool name, session id, and whether the result was truncated or cached),
plus marker spans for compaction, errors, oracle escalations, and steering.
